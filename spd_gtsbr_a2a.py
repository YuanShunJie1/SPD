...
from dataloader_gtsbr import gtsbr_dataloader
...
parser = argparse.ArgumentParser(description='PyTorch GTSBR Training')
...
parser.add_argument('--resume_path', default='checkpoint_gtsrb_all2all', type=str, help='path to dataset')
parser.add_argument('--dataset', default='gtsbr', type=str)
...
acc_net_log=open(f'{args.resume_path.strip()}/model_{args.model_type}_dataset=gtsbr_poison={args.epoch_poison}_dataset={args.dataset}_alpha={args.alpha}_lr={args.lr}_dev={args.dev}_acc.txt','w') 
asr_net_log=open(f'{args.resume_path.strip()}/model_{args.model_type}_dataset=gtsbr_poison={args.epoch_poison}_dataset={args.dataset}_alpha={args.alpha}_lr={args.lr}_dev={args.dev}_asr.txt','w')     
...
gtsrb = gtsbr_dataloader(batch_size=args.batch_size, num_workers=5)

trainloader = gtsrb.run(mode='train')
testloader  = gtsrb.run(mode='test')

trainloader1, train_only_target_loader, train_except_target_loader = gtsrb.run(mode='poison')
test_except_target_loader, test_only_target_loader = gtsrb.run(mode='poison_test')
...
def create_model(model_type=args.model_type):
    if model_type == 'resnet18':
        ...
        clean_model_path = 'model_resnet18_dataset=gtsrb_clean.pth'

    if model_type == 'vgg16':
        model = vgg16(num_classes=args.num_class)
        clean_model_path = 'model_vgg16_dataset=gtsbr_clean.pth'
        
    if model_type == 'mobilenetv2':
        model = mobilenetv2(num_classes=args.num_class) 
        clean_model_path = 'model_mobilenetv2_dataset=gtsbr_clean.pth'
...
def gradcam_visualize(model):
    ...
    mean = torch.tensor(MEAN_gtsbr)
    std = torch.tensor(STD_gtsbr)
    ...
    for _, (inputs_c, labels_c) in enumerate(test_except_target_loader):
        ...
        with GradCAM(model=model, target_layers=target_layers) as cam:
            for i in range(inputs_c.size(0)):
                ...
                visualization_c = show_cam_on_image(denorm_img_c, grayscale_cam_c, use_rgb=True, image_weight=0.6)
                ...
                visualization_b = show_cam_on_image(denorm_img_b, grayscale_cam_b, use_rgb=True, image_weight=0.6)
...
if __name__ == '__main__':
    ...
    torch.save(model.state_dict(), f'{args.resume_path.strip()}/model_{args.model_type}_dataset=gtsbr_poison={args.epoch_poison}_dataset={args.dataset}_alpha={args.alpha}_lr={args.lr}_dev={args.dev}_pth')
    torch.save(tgtmodel.state_dict(), f'{args.resume_path.strip()}/tgtmodel_{args.attack_model}_dataset=gtsbr_poison={args.epoch_poison}_dataset={args.dataset}_alpha={args.alpha}_lr={args.lr}_dev={args.dev}_pth')

    # model.load_state_dict(torch.load(f'{args.resume_path.strip()}/model_{args.model_type}_dataset=gtsbr_poison={args.epoch_poison}_dataset={args.dataset}_alpha={args.alpha}_lr={args.lr}_dev={args.dev}_pth'))
    # tgtmodel.load_state_dict(torch.load(f'{args.resume_path.strip()}/tgtmodel_{args.attack_model}_dataset=gtsbr_poison={args.epoch_poison}_dataset={args.dataset}_alpha={args.alpha}_lr={args.lr}_dev={args.dev}_pth'))
