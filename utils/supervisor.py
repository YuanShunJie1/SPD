...
def get_model_dir(args, cleanse=False, defense=False):
    # if hasattr(args, 'model_path') and args.model_path is not None:
    _index = [i for i, char in enumerate(args.model_path) if char == '/']
    return args.model_path[:_index[-1]]
    # else:
    # if not os.path.exists("fixed_models"):
    #     os.makedirs("fixed_models")
    # return f"{get_model_name(args, cleanse=cleanse, defense=defense)}"
...
def get_transforms(args):
    ...
    elif args.dataset == 'ember' or args.dataset == 'imagenet12':
...
def get_poison_transform(poison_type, dataset_name, target_class, source_class=1, cover_classes=[5, 7],
    ...
    if dataset_name in ['gtsrb', 'cifar10', 'cifar100']:
        img_size = 32
    elif dataset_name == 'imagenette' or dataset_name == 'imagenet' or dataset_name == 'imagenet12':
        img_size = 224
    else:
